...
class ProcessData():
    ...
    def determinar_municipios_para_execucao(self):
        ...
        ls = self.get_db()['licitacao'].distinct('cdIBGE')
        return ls

    def determinar_nr_ano(self):
        ls = self.get_db()['licitacao'].distinct('nrAnoLicitacao')
        return ls
    ...
    def determinar_nome_municipio(self,cd_ibge = None):
        ...
        obj = self.get_db().licitacao.find_one({'cdIBGE' : cd_ibge}, {'_id' : 0, 'nmMunicipio' : 1})
        if obj :
