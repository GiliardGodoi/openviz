...
class ProcessData():
    ...
    def complemento_informacao_entidade(self,cd_entidade,nr_ano):
        ...
        query = {'idEntidade' : cd_entidade, 'nrAnoLicitacao' : nr_ano}
        project = {"_id" : 0, 'cdIBGE' : 1, 'nmMunicipio' : 1, 'nmEntidade' : 1, 'idEntidade' : 1, 'nrAnoLicitacao' : 1}
        aux = self.get_db().licitacao.find_one(query,project)
        # pprint.pprint(aux)
    ...
    def executar_consulta(self,pipeline = None):
        ...
        cursor = self.get_db().licitacao.aggregate(pipeline)
        return cursor
